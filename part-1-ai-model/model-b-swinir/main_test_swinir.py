...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        output = np.transpose(output if output.shape[2] == 1 else output[:, :, [2, 1, 0]], (2, 0, 1))
        output = torch.from_numpy(output).float().unsqueeze(0).to(device)

        with torch.no_grad():
            # pad input image to be a multiple of window_size
            _, _, h_old, w_old = output.size()
            h_pad = (h_old // window_size + 1) * window_size - h_old
            w_pad = (w_old // window_size + 1) * window_size - w_old
            output = torch.cat([output, torch.flip(output, [2])], 2)[:, :, :h_old + h_pad, :]
            output = torch.cat([output, torch.flip(output, [3])], 3)[:, :, :, :w_old + w_pad]
            output1 = test(output, model, args, window_size)
            output1 = output1[..., :h_old * args.scale, :w_old * args.scale]
        output1 = output1.data.squeeze().float().cpu().clamp_(0, 1).numpy()
        if output1.ndim == 3:
            output1 = np.transpose(output1[[2, 1, 0], :, :], (1, 2, 0))
              # CHW-RGB to HCW-BGR
        output1 = (output1 * 255.0).round().astype(np.uint8)  # float32 to uint8
        cv2.imwrite(f'{save_dir}/{imgname}_SwinIR.png', output1)
